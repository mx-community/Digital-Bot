...
if (!args[0]) return conn.sendMessage(m.chat, { text: `Ingrese el comando y la cantidad de *${currency2}* mas en mencionar a un usuario.\n\n• Por ejemplo:\n*#${command}* 30000 @tag` }, { quoted: m })
...
if (user.exp < count) return conn.sendMessage(m.chat, { text: `📍  No tienes esa cantidad de *${currency2}* para regalar.\n- Solo dispones de *${user.exp}* de *${currency2}*.` }, { quoted: m })
user.exp -= count
if (typeof recipient.exp !== 'number') recipient.exp = 0
recipient.exp += count   
if (isNaN(user.exp)) user.exp = 0
